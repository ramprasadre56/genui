...
import 'features/screens/design_options_screen.dart';
import 'features/screens/estimate_screen.dart';
import 'features/widgets/app_navigator.dart';
import 'features/widgets/global_progress_indicator.dart';

void main() {
  initLogging();
  runApp(const ProviderScope(child: LandscapeArchitectApp()));
}
...
final _router = GoRouter(
  ...
  routes: [
    ...
    GoRoute(
      path: '/design_options',
      builder: (context, state) => const DesignOptionsScreen(),
    ),
    GoRoute(
      path: '/estimate',
...
class LandscapeArchitectApp extends ConsumerWidget {
  const LandscapeArchitectApp({super.key});
